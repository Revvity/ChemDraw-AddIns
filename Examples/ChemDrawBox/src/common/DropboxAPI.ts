   ...
   public getUserInfo(): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            if (this.accessToken.length === 0) {
                ...
                request.open("POST", "https://api.dropboxapi.com/2/users/get_current_account");
   ...
   public downloadFile(filePath: string): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            if (this.accessToken.length === 0) {
                ...
                httpRequest.open("POST", "https://content.dropboxapi.com/2/files/download");
                
                httpRequest.setRequestHeader("Authorization", 'Bearer ' + this.accessToken);
                httpRequest.setRequestHeader("Dropbox-API-Arg", JSON.stringify({ path: filePath }));
